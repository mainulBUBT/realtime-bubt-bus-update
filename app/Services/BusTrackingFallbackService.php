...
class BusTrackingFallbackService
{
    ...
    private function createSingleTrackerStatus(string $busId, ?BusCurrentPosition $currentPosition): array
    {
        ...
        return [
            ...
            'data_quality' => $singleUserData['data_quality'],
            'last_updated' => now(),
    ...
    private function analyzeSingleUserDataQuality($locations): array
    {
        ...
        $issues = [];
        if ($accuracyScore < 0.5) $issues[] = 'poor_gps_accuracy';
        if ($consistencyScore < 0.5) $issues[] = 'inconsistent_movement';
        if ($movementScore < 0.5) $issues[] = 'unrealistic_movement';
    ...
    private function calculateSingleUserConfidence(array $dataQuality): float
    {
        ...
        $qualityBonus = $dataQuality['overall_score'] * 0.3;
    ...
    private function getSingleUserWarningFlags(array $dataQuality): array
    {
        ...
        if ($dataQuality['overall_score'] < 0.3) {
            $flags[] = 'low_data_quality';
        }
        
        if ($dataQuality['location_count'] < 3) {
    ...
    private function calculateAccuracyScore($locations): float
    {
        ...
        if ($avgAccuracy <= 10) return 1.0;
        if ($avgAccuracy <= 25) return 0.8;
    ...
    {
        if ($locations->count() < 2) return 0.5;
    ...
    private function calculateMovementScore($locations): float
    {
        ...
        if ($avgSpeed >= 5 && $avgSpeed <= 60) return 1.0;
        if ($avgSpeed >= 2 && $avgSpeed <= 80) return 0.7;
        if ($avgSpeed >= 1 && $avgSpeed <= 100) return 0.4;
