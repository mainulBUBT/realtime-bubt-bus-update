...
<div class="mb-6 md:mb-8">
    <h1 class="text-2xl md:text-3xl font-bold text-gray-900 dark:text-white">Add Schedule</h1>
    <p class="text-gray-600 dark:text-gray-400 mt-1">Assign a bus to a route with a specific time</p>
</div>
...
<div class="bg-white dark:bg-gray-800 rounded-2xl shadow-sm hover:shadow-lg transition-all duration-300 border border-gray-100 dark:border-gray-700 p-6 md:p-8">
    <form action="{{ route('admin.schedules.store') }}" method="POST" class="space-y-6" data-form-submit>
        @csrf
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <select class="w-full px-4 py-3.5 border-2 border-gray-200 dark:border-gray-600 rounded-xl focus:outline-none focus:border-purple-500 focus:ring-4 focus:ring-purple-500/20 transition-all dark:bg-gray-700 dark:text-white appearance-none cursor-pointer @error('schedule_period_id') border-red-500 @enderror"
                    ...
                    @foreach($periods as $period)
                        <option value="{{ $period->id }}" {{ old('schedule_period_id') == $period->id ? 'selected' : '' }}>
            ...
            <div>
                ...
                <input class="w-full px-4 py-3.5 border-2 border-gray-200 dark:border-gray-600 rounded-xl focus:outline-none focus:border-purple-500 focus:ring-4 focus:ring-purple-500/20 transition-all dark:bg-gray-700 dark:text-white @error('departure_time') border-red-500 @enderror"
                       id="departure_time" type="time" name="departure_time" value="{{ old('departure_time') }}" required>
            ...
            <div>
                ...
                <input class="w-full px-4 py-3.5 border-2 border-gray-200 dark:border-gray-600 rounded-xl focus:outline-none focus:border-purple-500 focus:ring-4 focus:ring-purple-500/20 transition-all dark:bg-gray-700 dark:text-white @error('effective_date') border-red-500 @enderror"
                       id="effective_date" type="date" name="effective_date" value="{{ old('effective_date') }}">
        ...
        <div class="flex items-center gap-3 p-4 bg-gray-50 dark:bg-gray-700/30 rounded-xl">
            <input type="checkbox" name="is_active" value="1" {{ old('is_active') ? 'checked' : '' }} id="is_active" class="w-5 h-5 text-purple-600 border-gray-300 rounded focus:ring-purple-500 focus:ring-2">
